...
export default connect(mapStateToProps)((props) => {
        ...
        if (resp.payload.payload[0] && resp.payload.payload.length) {
          setChartData({
            labels: resp.payload.payload
              .map((bmi) => (bmi.record_date ? toDate(bmi.record_date) : ""))
              .reverse(),
            datasets: [
              {
                data: resp.payload.payload
                  .map((bmi) =>
                    bmi.value != "" && bmi.value != "NaN"
                      ? parseFloat(bmi.value.replace(/[\[\]&]+/g, "").trim())
                      : 0
                  )
                  .reverse(),
              },
  ...
  useEffect(() => {
    if (response) {
      const min = getMinDate(response.map((bmi) => new Date(bmi.record_date)));
      const max = getMaxDate(response.map((bmi) => new Date(bmi.record_date)));
      if (min) {
        max.setHours(9);
        min.setHours(9);
        setMinDate(min);
      ...
      <ScrollView>
        <View style={styles.scrollViewContainer}>
          <Text style={styles.scrollViewHeader}>
            {i18n.t("health.bmi.bodyMassIndex")}
          </Text>
          <FilterIcon
        ...
        (chartData.datasets && !chartData.datasets[0].data.length) ? (
          ...
          <>
            ...
            <View style={styles.contentContainer}>
              {/* <AddIcon>{i18n.t("health.bmi.addBMI")}</AddIcon> */}
              <Statistics data={filteredResponse} decimalPlaces={2} />
            </View>
          </>
        )}
      </ScrollView>
