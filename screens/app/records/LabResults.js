...
import {
  getAllResults,
  getResultsTypes as requestResultTypes,
...
import { LabResults } from "./common/types";
...
export default connect(mapStateToProps)((props) => {
  ...
  const getAllData = () => {
    requestResultTypes({ sessionKey: userData.sessionKey }).then((data) => {
      setIsLinked(myProviders ? myProviders.length > 0 : true);

      if (data.payload.success) {
        if (data.payload.payload && data.payload.payload.length > 0)
          dispatch(startLoading());
        setResultsTypes(data.payload.payload);
        const filteredTypes = Array.from(data.payload.payload).map((o) => true);
        setFilteredResultsTypes(filteredTypes);
        getResultsData(data.payload.payload, filteredTypes);
      } else setLoaded(true);
    });
  };
  const getResultsData = (resultsTypes, filteredResultsTypes) => {
    getAllResults({
      sessionKey: userData.sessionKey,
      categories: resultsTypes.filter(
        (_, index) => filteredResultsTypes[index]
      ),
    })
      .then((resp) => {
        setLoaded(true);
        if (resp.payload.success) {
          setResults(resp.payload.payload[0]);
        } else setResults([]);
      })
      .catch((error) => {
        console.warn(error);
      ...
      .filter((o) =>
        search
          ? o.name.toLowerCase().indexOf(search.toLowerCase()) > -1 ||
            ...
            (o.value &&
              o.value.toLowerCase().indexOf(search.toLowerCase()) > -1)
          : true
      )
      .map((o) => o.category)
    ...
    (o) =>
      (search
        ? o.name.toLowerCase().indexOf(search.toLowerCase()) > -1 ||
          ...
          (o.category &&
            o.category.toLowerCase().indexOf(search.toLowerCase()) > -1) ||
          (o.value && o.value.toLowerCase().indexOf(search.toLowerCase()) > -1)
        : true) &&
      resultsTypes
  ...
  // useEffect(() => {
  //   if (filteredData.length != 0 && resultsTypes.length != 0) {
  //     // let resultsData = []
  //     resultsTypes.forEach(cat => finalResults.push({ name: cat, res: filteredData.filter(function (result) { return result.category === cat }) }));
  //     // finalResults.push(resultsData);
  //   }
  // }, [filteredData]);
  ...
  if (wrapper === false) {
    wrapper = (
      <RecordList data={finalResults} useListItem={true} type={LabResults} />
    );
  }
  return (
    <PageContainer>
      <FilterModal
        ...
        headerTitle={i18n.t("records.labResults.filterType")}
        errorMessage={i18n.t("records.labResults.selectFilter")}
        onClose={() => {
          setModalVisible(false);
        }}
        onFilter={() => {}}
