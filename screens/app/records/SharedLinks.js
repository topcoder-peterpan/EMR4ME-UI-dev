      ...
      getAllLinks(params)
        .then((data) => {
          if (data.statusCode == 0) {
            ...
            sortedLinks = sortedLinks.filter(l => res.payload.find(d => d.id == l.doctor_id));
    ...
    getLinkByID({ linkId: item.link_id })
      .then((data) => {
        if (data.statusCode == 0) {
          item = data.payload;
          let sharedData = {};
          let doctorName = (route.params && route.params.name) || getDoctorName(item.doctor_id);
          sharedData[Doctor] = [
    ...
    return (
      <>
        ...
        <SharedLinksBottomSheet
          ...
          openExpiryDateModal={openExpiryDateModal}
          openLink={openLink}
          openConfirmDeleteDialogue={openConfirmDeleteDialogue}
